...
def read_h5_native_file(file, stream=True, channels=[0, -1], auxiliary=True):
    ...
    with h5py.File(file, "r") as f:
        ...
        # Data is in the format of (nsamp, nchan).
        # So a single trace is dset[:,tracenumber]
        if channels == [0, -1]:
            data = np.array(dset[:, :])
        elif len(channels) > 2:
...
def read_h5_native_files(files, as_stream=True, stream=True, channels=[0, -1],
    ...
    for i in range(len(files)-1):
        ...
        if (fs0 == fs1) & (dx0 == dx1) & (d00 == d01):
            ...
            warnings.warn(("Different acquisition parameters. \n>> Falling"
                           + "back to as_stream=True and stream=True with"
                           + "merge=False"), UserWarning)
            as_stream = True
            stream = True
            merge = False
            break
    ...
    if as_stream:
        ...
        if stream:
            if not merge:
                warnings.warn("merge=True was set because of working with"
                              + "the numpy arrays", UserWarning)
            fs = int(metadata["SamplingFrequency[Hz]"])
            dx = float(metadata["SpatialResolution[m]"])
            d0 = float(metadata["Start Distance (m)"])
            starttime = UTCDateTime(str(metadata["ISO8601 Timestamp"]))
...
def read_h5_asdf_files(files, stream=True, channels=[0, -1], auxiliary=True,
                       merge=True, sort=True):
    """Subroutine to read in asdf files. Called by das_reader, so please look
    ...
    for i in range(len(st0)):
        ...
        if check_stats:
            if not st0[i].stats == st1[i].stats:
                warnings.warn("\n>>Trace stats are different!", UserWarning)
                print("stats")
                return False
        else:
            pass
